...
class MainTest {
    ...
    fun `journey time is calculated correctly if there is three shops in the string that are reachable in the same day`() {
        assertEquals(21509 + 2 * minTimeSpentAtEachShop, calculateJourneyTime(peterJonesHeadOfficeAndLiverpoolAsString))
    }
    ...
    fun `journey time is calculated correctly if array of shops contains many shops with one in the middle unreachable `(){
        ...
        val timeAllowedEachDay = 6000
        val timeRequiredForBreak = 10000

        val expectedTime = oneDay + oneHour + minTimeSpentAtEachShop
        assertEquals(expectedTime , listOf(shop1, shop2, shop3, shop4, shop5, shop6).calculateJourneyTime(timeAllowedEachDay,timeRequiredForBreak))
    }
    ...
    fun `Test with the data from the challenge`() {
        ...
        assertEquals(1192181, calculateJourneyTime(allShopData))
    }
    @Test
