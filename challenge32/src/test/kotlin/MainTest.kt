...
class MainTest {
    @Test
    fun `standby claim for 1 day on Mon to Thursday which is not a holiday is weekday rate`() {
    ...
    @Test
    fun `standby claim for 1 day on Friday which is not a holiday is weekend rate`() {
        val result = calcStandbyClaim(LocalDate.parse("2021-04-16"),1,"A")
        val expectedResult = Claim(WEEKEND_RATE_A,listOf("2021-04-16, Weekend rate A, £40"))
        assertEquals(expectedResult, result)
    }

    @Test
    fun `standby claim for 1 day on Saturday or Sunday is twice the weekend rate`() {
        val saturdayResult = calcStandbyClaim(LocalDate.parse("2021-04-17"),1,"A")
        ...
        val sundayResult = calcStandbyClaim(LocalDate.parse("2021-04-18"),1,"A")
    ...
    @Test
    fun `standby claim for 1 day on a bankholiday is twice the bankholiday rate`() {
        val result = calcStandbyClaim(LocalDate.parse("2021-04-02"),1,"A")
