import example3Point1.example3point1
import model.*
...
class TestExample3point1 {
    ...
    fun `test initial version`() {
        val configuration =  example3point1()
        ...
        assertEquals(true, acid_bath.uses[0] is Electricity)
        val acidBathElectricity = acid_bath.uses[0] as Electricity
        assertEquals(12, acidBathElectricity.power)
        assertEquals(true, acid_bath.uses[1] is Acid)

        val acid = acid_bath.uses[1] as Acid
