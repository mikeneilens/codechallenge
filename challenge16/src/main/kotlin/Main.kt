...
val String.fromRomanSymbolToInt:Int get() {
    val result:Int =  this.toList().map{it.toString()}.foldRight(Pair(0,0),{ romanDigit, acc ->
        val (total, valueOfLastRomanDigit) = acc
...
val Int.fromIntToRomanSymbol: String get() =
    romanKeyValues.toList().fold(Pair("",this)){ (result,remainder),romanKeyValue ->
        val (symbol, value) = romanKeyValue
